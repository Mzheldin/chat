...
public class BaseAuthService implements AuthService {
    ...
    private static Logger log = Logger.getLogger(BaseAuthService.class.getName());
...
//        users.put("nick3", new User("login3", "pass3", "nick3"));
        try {
            ...
            log.log(Level.ALL,"Ошибка инициализации", e);
        } finally {
            disconnect();
        }
    }
    ...
    public static void disconnect() {
        try {
            ...
            log.log(Level.ALL, "Ошибка закрытия запроса к базе", e);
        }
        try {
            connection.close();
        } catch (Exception e) {
            log.log(Level.ALL,"Ошибка закрытия подключения к базе данных", e);
        }
    }
    ...
    public void changeLogin(String login, String nick) {
        try {
            ...
            log.log(Level.ALL,"Ошибка обновления логина", e);
        } finally {
            disconnect();
        }
    }
    ...
    public User createOrActivateUser(String login, String password, String nick) {
        ...
        if (users.containsKey(nick)) {
            ...
            log.info("User with nick " + nick + "already exist");
        } else {
            users.put(nick, user);
            persist(user);
        }
    ...
    private void persist(User user) {
        try {
            ...
            log.log(Level.ALL, "Ошибка при добавлении пользователя", e);
        } finally {
            disconnect();
        }
    }
